        ...
        IAnalysisHostedService<AnalysedComponent>
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    {
                        var xAnalysedComponentEvent = scope.ServiceProvider
                            .GetRequiredService<IXAnalysedComponentEvent>();
                        
                        var currentBatch = xAnalysedComponentEvent
                            .GetNextWorkingSet(0, true);
        ...
        public bool Analyse(AnalysedComponent entity)
        {
            if (entity != null)
            {
                using (var scope = _scopeFactory.CreateScope())
                {
                    var analysedComponentEvent = scope.ServiceProvider.GetRequiredService<IAnalysedComponentEvent>();
                    var analysedHistoricItemEvent = scope.ServiceProvider
                        .GetRequiredService<IAnalysedHistoricItemEvent>();
                    var currencyEvent = scope.ServiceProvider.GetRequiredService<ICurrencyEvent>();
                    var currencyPairEvent = scope.ServiceProvider.GetRequiredService<ICurrencyPairEvent>();
                    var processAnalysedComponentService = scope.ServiceProvider.GetRequiredService<IProcessAnalysedComponentService>();
                    
                    if (entity.ModifiedAt.AddMilliseconds(entity.Delay) > DateTime.UtcNow)
                    {
                        processAnalysedComponentService.Checked(entity.Id, true);
                        _logger.LogWarning(
                            $"[{ServiceName}] Analyse: Skipping {entity.Id}, has been recently updated.");
                        return true;
                    }
                    ...
                    switch (entity.ComponentType)
                    {
                        case AnalysedComponentType.Unknown:
                            ...
                            return processAnalysedComponentService.Checked(entity.Id);
                        ...
                        case AnalysedComponentType.MarketCap:
                            ...
                            if (entity.CurrencyTypeId != null && entity.CurrencyTypeId > 0)
                            {
                                switch (entity.ComponentType)
                                {
                                    ...
                                    case AnalysedComponentType.DailyMarketCap:
                                        ...
                                        var obtainedComponent = analysedComponentEvent
                                        ...
                                        if (obtainedComponent != null &&
                                            obtainedComponent.AnalysedHistoricItems.Count > 0)
                                        {
                                            ...
                                            return processAnalysedComponentService.UpdateValue(entity.Id,
                                                obtainedComponent.AnalysedHistoricItems
                                    ...
                                    case AnalysedComponentType.MarketCap:
                                        ...
                                        var analysedComponents = analysedComponentEvent.GetAllCurrencyComponentsByType(
                            ...
                            else if (entity.CurrencyId != null && entity.CurrencyId > 0)
                            {
                                ...
                                var currencyAveragePrice = analysedComponentEvent.GetAllByCurrency(
                                ...
                                if (currencyAveragePrice != null)
                                {
                                    ...
                                    var circulatingSupply = currencyEvent.GetCirculatingSupply(entity);

                                    // Parsable average?
                                    if (circulatingSupply > 0
                                        && decimal.TryParse(currencyAveragePrice.Value, out var avgPrice) &&
                                        avgPrice > 0)
                                    {
                                        // Market Cap Formula
                                        var marketCap = decimal.Multiply(circulatingSupply, avgPrice);

                                        return processAnalysedComponentService.UpdateValue(entity.Id, marketCap
                                            .ToString(CultureInfo.InvariantCulture));
                                    }
                                }
                            }
                            ...
                            {
                                var circulatingSupply = currencyEvent.GetCirculatingSupply(entity);
                                var analysedComponents = analysedComponentEvent.GetAllByCorrelation(entity.Id,
                        ...
                        case AnalysedComponentType.CurrentAveragePrice:
                            ...
                            else if (entity.CurrencyId != null && entity.CurrencyId > 0)
                            {
                                ...
                                var components = currencyEvent.GetTickerPairComponents(
                                    (long) entity.CurrencyId,
                                ...
                                while (components.Any()) // iterate
                                {
                                    ...
                                    components = currencyEvent.GetTickerPairComponents(
                                        (long) entity.CurrencyId,
                                        // Ensure that all components used are valid, no historical values are being tapped on.
                                        true, ++index, false, ac => // Make sure its the generic counter currency
                                            // since we can't convert yet
                            ...
                            else
                            {
                                ...
                                var index = 0;
                                var components = currencyPairEvent.GetComponents(entity.Id, true,
                                ...
                                return processAnalysedComponentService.Checked(entity.Id, true);
                            }
                        ...
                        case AnalysedComponentType.HourlyAveragePrice:
                            ...
                            {
                                var compData = analysedHistoricItemEvent.TraverseRelatedHistory(entity.Id,
                                    AnalysedComponentType.CurrentAveragePrice);

                                // Safetynet
                                if (compData.Data == null)
                                {
                                    processAnalysedComponentService.Checked(entity.Id);
                                    return true;
                                }
                                ...
                                for (var i = 0; i < compData.Pages; i++)
                                {
                                    if (i > 0)
                                    {
                                        var currData = analysedHistoricItemEvent.TraverseRelatedHistory(entity.Id,
                            ...
                            else
                            {
                                ...
                                var componentsToCompute = analysedHistoricItemEvent.GetRelevantComponentQueryCount(
                                    entity.Id,
                                    // Active checks
                                ...
                                {
                                    if (currencyPairEvent.HasRelatedComponent(entity.Id,
                                        AnalysedComponentType.CurrentAveragePrice))
                                    {
                                        // Obtain all the historic items related to this AC.
                                        var analysedComponent = currencyPairEvent.GetRelatedAnalysedComponent(
                                            entity.Id,
                        ...
                        case AnalysedComponentType.DailyAveragePrice:
                            ...
                            else if (entity.CurrencyId != null && entity.CurrencyId > 0)
                            {
                                ...
                                {
                                    var historicData = analysedHistoricItemEvent.GetAll(entity.Id, dataTimespan, i)
                            ...
                            else
                            {
                                ...
                                {
                                    // Obtain all the historic items related to this AC.
                                    var analysedComponent = currencyPairEvent.GetRelatedAnalysedComponent(entity.Id,
                        ...
                        case AnalysedComponentType.DailyPricePctChange:
                            ...
                            // How many components we got
                            var pctChangeComponentsToCompute =
                                analysedHistoricItemEvent.GetRelevantComponentQueryCount(
                                    entity.Id,
                                    // Active checks
                                    ahi => ahi.DeletedAt == null && ahi.IsEnabled
                            ...
                            if (pctChangeComponentsToCompute > 0)
                            {
                                ...
                                if (entity.CurrencyId != null && entity.CurrencyId > 0)
                                {
                                    ...
                                    {
                                        var historicData = analysedHistoricItemEvent.GetRelevantHistorics(entity.Id,
                                                ahi => ahi.DeletedAt == null && ahi.IsEnabled
                                                                             // Time check
                                                                             && ahi.HistoricDateTime >=
                                                                             DateTime.UtcNow.Subtract(dataTimespan)
